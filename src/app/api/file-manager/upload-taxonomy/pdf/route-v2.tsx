...
// - Detects bullet (•, -, – , —, ●, ▪) and numbered lists (1., 1), ๑.)
// - Detects Thai labels such as ชื่อวิทยาศาสตร์ / ชื่อพ้อง / ชื่ออื่น ๆ / วงศ์ / ผู้เขียนคำอธิบาย /
//   ชื่อผู้ตั้งพรรณพืช / ช่วงเวลาเกี่ยวกับผู้ตั้งพรรณพืช and wraps them as <p><strong>LABEL</strong> value</p>
function pdfTextToHtmlAdvanced(raw: string): string {
  ...
  const ORDERED_RE = /^\s*(\d+|[๐-๙]+)[\.)]\s+(.*)$/u; // Arabic or Thai numerals

  // Match common Thai labels (tone/space tolerant variants via minimal regex)
  ...
  while (i < lines.length) {
    ...
    // Labeled paragraph. Merge following continuation lines (until blank/new block)
    ...
    // Regular paragraph — merge until blank or a new structural block starts
...
// Persist original imported HTML into Taxon.* (supports several field names)
...
async function extractFromPdf(buffer: Buffer): Promise<{ html: string; text: string }> {
  try {
    ...
    // Use the advanced converter instead of simple paragraph-wrapping
    const html = pdfTextToHtmlAdvanced(text);
    return { html, text };
  } catch (e: any) {
    // If this is our ModuleMissingError, let the caller handle it; otherwise rethrow
    if (e && typeof e === 'object' && (e as any).code === 'MODULE_MISSING') {
      throw e;
    }
    // Unexpected error while parsing PDF (e.g., corrupt file) — surface a friendlier message
    console.error('[upload-taxonomy] pdf parsing failed:', e);
    throw new Error('ไม่สามารถอ่านเนื้อหา PDF ได้: ไฟล์อาจเสียหายหรือรูปแบบไม่รองรับ');
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Convert to HTML/text depending on file type
