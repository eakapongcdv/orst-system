...
function collapseDuplicateThaiVowels(s: string): string {
  ...
  // จัดเรียง code points ให้ถูกต้อง
  return s.replace(
    /([\u0E30\u0E31\u0E32\u0E33\u0E34-\u0E3A\u0E47-\u0E4E])\1+/gu,
    '$1'
  );
...
function pdfTextToHtmlAdvanced(raw: string): string {
  ...
  const LABEL_PATTERNS: Array<{ label: string; re: RegExp }> = [
    ...
    // อัปเดตล่าสุด: เปลี่ยน label จาก "ผู้เขียนคำอธิบาย" เป็น "ผู้เขียน"
    { label: 'ผู้เขียน', re: /^\s*ผู้\s*เขียน\s*[:\-–—]?\s*(.+)$/u },
  ...
  for (const f of candidates) {
    try {
      await prisma.taxon.update({
        where: { id: taxonId },
        data : { [f]: html } as any, // <-- แก้ไข: เพิ่ม '' ตรงนี้
      });
      return f;
    } catch {
...
async function loadPdfJs(): Promise<any> {
  ...
  // --- Attempt to load pdfjs-dist (try common entry points) ---
  try {
    // Try modern ESM entry point first (common in newer versions)
    pdfjsLib = await import('pdfjs-dist/legacy/build/pdf.mjs');
  } catch (e1) {
    errs.push(e1);
    try {
      // Fallback to standard ESM entry point
      pdfjsLib = await import('pdfjs-dist/build/pdf.mjs');
    } catch (e2) {
      errs.push(e2);
      try {
        // Fallback to legacy CJS/JS entry point
        pdfjsLib = await import('pdfjs-dist/legacy/build/pdf.js');
      } catch (e3) {
        errs.push(e3);
        try {
          // Final fallback to main package entry (might be CJS or ESM depending on version/bundler)
  ...
  if (!pdfjsLib) {
    console.error('[upload-taxonomy] pdfjs-dist load failed after trying multiple entry points:', ...errs);
  ...
  // Use createRequire to find the path to the worker file within the pdfjs-dist package.
  // This avoids CDN and potential "File URL host" issues.
  try {
    const req = createRequire(import.meta.url);
    let workerPath: string | null = null;

    // Try to resolve the .mjs worker first (newer format)
    try {
      workerPath = req.resolve('pdfjs-dist/legacy/build/pdf.worker.mjs');
      console.log(`[upload-taxonomy] Found local pdf.worker.mjs at: ${workerPath}`);
    } catch (mjsErr) {
      console.log('[upload-taxonomy] pdf.worker.mjs not found, trying .js worker...');
      // If .mjs fails, try the .js worker
      try {
        workerPath = req.resolve('pdfjs-dist/legacy/build/pdf.worker.js');
        console.log(`[upload-taxonomy] Found local pdf.worker.js at: ${workerPath}`);
      } catch (jsErr) {
        console.log('[upload-taxonomy] pdf.worker.js not found, trying build/ path...');
        // If legacy/build fails, try build/ path
        try {
          workerPath = req.resolve('pdfjs-dist/build/pdf.worker.mjs');
          console.log(`[upload-taxonomy] Found local pdf.worker.mjs (build/) at: ${workerPath}`);
        } catch (buildMjsErr) {
          try {
            workerPath = req.resolve('pdfjs-dist/build/pdf.worker.js');
            console.log(`[upload-taxonomy] Found local pdf.worker.js (build/) at: ${workerPath}`);
          } catch (buildJsErr) {
            console.warn('[upload-taxonomy] Could not resolve local pdf.worker file. pdfjs-dist might use a different structure or fail later.');
            // If all fail, workerPath remains null, and we let pdf.js handle it (might use fake worker or fail during doc processing)
          }
        }
      }
    }

    // If a worker path was successfully resolved, set it
    if (workerPath) {
      // Ensure the path is absolute and uses the file:// protocol for pdf.js compatibility
      const absoluteWorkerPath = workerPath.startsWith('file://') ? workerPath : `file://${workerPath}`;
      pdfjsLib.GlobalWorkerOptions.workerSrc = absoluteWorkerPath;
      console.log(`[upload-taxonomy] Configured pdfjs-dist to use local worker: ${absoluteWorkerPath}`);
    } else {
      console.warn('[upload-taxonomy] No local worker path found. pdfjs-dist might fall back to a fake worker or fail.');
    }
  } catch (workerSetupError) {
    console.warn('[upload-taxonomy] Warning during local worker setup:', workerSetupError);
    // Do not throw, let pdf.js attempt to proceed or fail during document processing
  }
...
function thaiKey(s: string): string {
  return normThaiBasic(s).replace(/[\uE31\uE34-\uE4E]/g, '').replace(/\s+/g, '');
}
...
function cleanImportedHtml(html: string): string {
  ...
  try {
    ...
    for (const p of paras) {
      ...
      // ป้องกันการลบเนื้อหาที่มี '?' มากเกินไป ถ้า normThaiBasic ทำงานก่อน จะลดโอกาสลง
...
// --- แทนที่ elementContainsStrongAuthor ---
// ตรวจสอบว่า Element นี้เป็นจุดที่ควรตัด entry หรือไม่
// - กรณี 1: มี <strong> ที่ขึ้นต้นด้วย "ผู้...เขียน"
// - กรณี 2: มี <strong> ที่ขึ้นต้นด้วย "ภาพ...ประกอบ"
function isSplitMarkerElement(el: Element): boolean {
  // อนุญาตตัวไทย ช่องว่าง และอักขระ zero-width/FFFD คั่นระหว่าง “ผู้” … “เขียน” หรือ “ภาพ” … “ประกอบ”
  const THAI_ANY = '[\\u0E00-\\u0E7F\\s\\u200B\\u200C\\u200D\\uFEFF\\uFFFD]*';
  ...
  // ต้องขึ้นต้นด้วย “ภาพ…ประกอบ” อาจมี : - – — คั่น แล้วตามด้วยชื่อผู้ประกอบภาพอย่างน้อย 1 คำ
  const STRONG_ILLUSTRATOR_RE = new RegExp(
    `^ภาพ${THAI_ANY}ประกอบ(?:\\s*[:\\-–—])?\\s+\\S+`,
    'u'
  );
  ...
  for (const s of strongNodes) {
    ...
    if (STRONG_AUTHOR_RE.test(text) || STRONG_ILLUSTRATOR_RE.test(text)) {
      return true;
    }
  }
  return false;
}
// --- สิ้นสุดการแทนที่ elementContainsStrongAuthor ---

// --- แก้ไข splitEntriesByAuthorMarker ---
// Split entries whenever we encounter an element that *contains a STRONG*
// whose text roughly matches "ผู้…เขียน" OR "ภาพ...ประกอบ" (with tolerance for corrupted glyphs).
function splitEntriesByAuthorMarker(html: string): Array<{ title: string; html: string; text: string }> {
  ...
  for (const el of elems) {
    // ตรวจสอบว่าเป็นจุดตัดหรือไม่ *ก่อน* ใส่ลงใน buffer
    if (isSplitMarkerElement(el)) {
      // ถ้า buffer มีอะไรอยู่ก่อน ให้ flush ก่อน
      // แล้วค่อยใส่ element นี้ (จุดตัด) เข้า buffer ใหม่ และ flush ทันที
      // เพื่อให้ entry ใหม่เริ่มต้นด้วยจุดตัดนี้
      flush(); // Flush สิ่งที่สะสมไว้ก่อนหน้า
      buf.push(el); // ใส่จุดตัดเข้า buffer ใหม่
      flush(); // Flush จุดตัดทันที เพื่อให้ entry ใหม่เริ่มจากจุดตัด
    } else {
       buf.push(el); // ใส่ element ปกติเข้า buffer
    }
  }
  // tail - สิ่งที่เหลือใน buffer หลังจากวนลูป
  flush();
...
// --- สิ้นสุดการแก้ไข splitEntriesByAuthorMarker ---
...
// --- อัปเดตล่าสุด: แก้ไข extractAuthorByRegex ---
function extractAuthorByRegex(html: string): string | undefined {
  ...
  // อัปเดตล่าสุด: ปรับ regex ให้ match "ผู้เขียน" แทน "ผู้เขียนคำอธิบาย"
  ...
  // อัปเดตล่าสุด: ปรับเงื่อนไขการตรวจสอบ label เป็น 'ผู้เขียน'
  try {
    ...
    for (const p of ps) {
      ...
      // อัปเดตล่าสุด: ตรวจสอบว่า label เริ่มต้นด้วย 'ผู้เขียน'
...
// --- สิ้นสุดการอัปเดต extractAuthorByRegex ---
...
function parseMetaFromHtml(html: string): {
  ...
  try {
    ...
    for (const p of ps) {
      ...
      // อัปเดตล่าสุด: ปรับเงื่อนไขให้ match กับ label ที่เปลี่ยนเป็น "ผู้เขียน"
      if ((/^ผู้เขียน$/.test(labelNoTone) || labelNoTone.includes('ผู้เขียน')) && contentText) {
...
function makeLabels(s: string) {
  ...
  const noTone = base.replace(/[\uE48-\uE4B]/g, '');
  return { base, noTone };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // --- อัปเดตล่าสุด: ปรับปรุงลำดับการ clean ---
    // 1. แทนที่ U+FFFD (glyph ผิดพลาด) ด้วย placeholder '?' ก่อน clean
    const htmlWithPlaceholder = htmlOutRaw.replace(/\uFFFD/g, '?');
    console.log('[DEBUG] POST - HTML after replacing FFFD with ?:', htmlWithPlaceholder.length, 'chars');

    // 2. ใช้ normThaiBasic เพื่อ clean text (รวมถึงแทนที่ pattern '?...' และบีบสระซ้ำ)
    //    ทำ *ก่อน* cleanImportedHtml เพื่อป้องกันการลบเนื้อหาที่มี '?' มากเกินไป
    const htmlNormThaiBasic = normThaiBasic(htmlWithPlaceholder);
    console.log('[DEBUG] POST - HTML after normThaiBasic:', htmlNormThaiBasic.length, 'chars');

    // 3. ใช้ cleanImportedHtml เพื่อลบ noise ที่เหลือ (เช่น .indd, หมายเลขหน้า)
    const htmlOut = cleanImportedHtml(htmlNormThaiBasic);
    console.log('[DEBUG] POST - Final htmlOut length after cleanImportedHtml:', htmlOut.length);

    // 4. ประมวลผล raw text (ใช้ normThaiBasic แยกต่างหาก)
    const rawOut = normThaiBasic((rawOutBase || ''));
    console.log('[DEBUG] POST - Final rawOut length:', rawOut.length);
    // --- สิ้นสุดการปรับปรุงลำดับการ clean ---
