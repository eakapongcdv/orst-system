...
/** Constraints: .html/.htm/.doc/.docx/.pdf */
const allowedTypes = [
  "text/html",
  "application/msword",
...
const allowedExt = [".html", ".htm", ".docx", ".doc", ".pdf"];
  ...
  const validateFile = (file: File): boolean => {
    ...
    if (!allowedTypes.includes(file.type) && !allowedExt.includes(ext)) {
      setError("ไฟล์ไม่รองรับ กรุณาอัปโหลดไฟล์ .html, .htm, .docx, .doc หรือ .pdf เท่านั้น");
      return false;
    }
  ...
  const uploadFile = async () => {
    ...
    try {
      await new Promise<UploadResult>((resolve, reject) => {
        ...
        const apiUrl = new URL("/api/file-manager/upload-taxonomy/pdf", window.location.origin);
            ...
            <div className="card-body">
              ...
              >
                <input
                  ref={fileInputRef}
                  type="file"
                  accept=".html,.htm,text/html,.doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,.pdf,application/pdf"
                  className="hidden"
